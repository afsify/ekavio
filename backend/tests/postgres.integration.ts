...
test('PostgreSQL shared-core migration, shadow copy, constraints, and verification', async (context) => {
  ...
  await context.test('migrates a clean database and reruns deterministically', async () => {
    assert.deepEqual((await getMigrationStatus(database)).map(({ state }) => state), ['pending', 'pending', 'pending', 'pending', 'pending', 'pending']);
    await migrate(database);
    await migrate(database);
    assert.deepEqual((await getMigrationStatus(database)).map(({ state }) => state), ['applied', 'applied', 'applied', 'applied', 'applied', 'applied']);
